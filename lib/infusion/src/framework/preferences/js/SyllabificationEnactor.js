...
fluid.prefs.enactor.syllabification.parse = function (that, elm) {
    ...
    elm = elm.nodeType === Node.ELEMENT_NODE ? jQuery(elm) : jQuery(elm.parentNode);
...
fluid.prefs.enactor.syllabification.hyphenateNode = function (hyphenator, node, separatorMarkup, softHyphenPlaceholderMarkup) {
    ...
    for (var i = 0; i < segs.length; i++) {
        ...
        if (node.textContent.startsWith(softHyphen)) {
            ...
            var placeholder = jQuery(softHyphenPlaceholderMarkup)[0];
        ...
        if (i < segs.length - 1) {
            var separator = jQuery(separatorMarkup)[0]; // converts the separatorMarkup markup string into a Dom Node
