...
// polyfill for jQuery.browser which was removed in jQuery 1.9 and later
...
fluid.getScopedData = function (target, key) {
    var data = jQuery(target).data("fluid-scoped-data");
...
fluid.setScopedData = function (target, key, value) {
    jQuery(target).each(function () {
        var data = jQuery.data(this, "fluid-scoped-data") || {};
        data[key] = value;

        jQuery.data(this, "fluid-scoped-data", data);
    });
};
...
fluid.enabled = function (target, state) {
    ...
    if (state === undefined) {
        ...
        jQuery("*", target).add(target).each(function () {
...
jQuery.each(["focus", "blur"], function (i, name) {
