...
// TODO: Coming soon - reimplementation of jQuery.extend using strategyRecursionBailout
fluid.extend = jQuery.extend;
 ...
 * passes through jQuery.extend shallow algorithm)
...
fluid.parsePriorityRecords = function (records, name) {
    var array = fluid.hashToArray(records, "namespace", function (newElement, oldElement) {
        jQuery.extend(newElement, oldElement);
...
fluid.resolveGrade = function (defaults, defaultName, gradeNames) {
    ...
    for (var i = 0; i < mergeArgs.length; ++i) {
        if (mergeArgs[i] && mergeArgs[i].mergePolicy) {
            mergePolicy = jQuery.extend(true, mergePolicy, mergeArgs[i].mergePolicy);
        }
    }
...
fluid.makeComponentCreator = function (componentName) {
    ...
    if (existing) {
        jQuery.extend(creator, existing);
    }
...
fluid.model.applyChangeRequest = function (model, request) {
    ...
    if (segs.length === 0) {
        if (request.type === "ADD") {
            jQuery.extend(true, model, request.value);
        } else {
            fluid.clear(model);
        }
...
fluid.mergeComponentOptions = function (that, potentia, lightMerge) {
    ...
    function computeMergePolicy() {
        ...
        mergePolicy = jQuery.extend({}, fluid.rootMergePolicy, mergePolicy);
        ...
        jQuery.extend(true, sharedMergePolicy, compiledPolicy.builtins); // ensure it gets broadcast to all sharers
    }
    computeMergePolicy();
...
fluid.mergeOneDistribution = function (target, source, key) {
    ...
    target[namespace] = jQuery.extend(true, {}, target[namespace], source);
};
