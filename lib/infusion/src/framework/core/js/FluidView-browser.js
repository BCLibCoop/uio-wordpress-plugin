...
// in jQuery 1.7.1, jQuery pulled the same dumb trick with jQuery.text() that they did with jQuery.val() previously,
...
fluid.findData = function (elem, name) {
    while (elem) {
        var data = jQuery.data(elem, name);
        if (data) {
            return data;
        }
...
fluid.bindFossils = function (node, data, fossils) {
    jQuery.data(node, fluid.BINDING_ROOT_KEY, {data: data, fossils: fossils});
};
...
fluid.jById = function (id, dokkument) {
    ...
    var togo = element ? jQuery(element) : [];
    togo.selector = "#" + id;
...
fluid.ariaLabeller.generateLiveElement = function (that) {
    var liveEl = jQuery(that.options.liveRegionMarkup);
    liveEl.prop("id", that.options.liveRegionId);
    jQuery("body").append(liveEl);
    return liveEl;
};
...
fluid.updateAriaLabel = function (element, text, options) {
    options = jQuery.extend({}, options || {}, {text: text});
...
fluid.deadMansBlur = function (control, options) {
    ...
    var that = {options: jQuery.extend(true, {}, fluid.defaults("fluid.deadMansBlur"), options)};
    ...
    fluid.each(that.options.exclusions, function (exclusion) {
        exclusion = jQuery(exclusion);
        fluid.each(exclusion, function (excludeEl) {
            jQuery(excludeEl).on("focusin", that.canceller).
    ...
    if (!that.options.cancelByDefault) {
        jQuery(control).on("focusout", function (event) {
